...
$organism =preg_replace('/\s+/', '',$_POST["org"]);
$replace = $_POST["replace"];
if($organism == "Others"){
	$organism=preg_replace('/\s+/', '',$_POST["org_other"]);
}
...
#	echo ("sudo bash /home/genomics/bin/cellranger.sh $folder_name $sequencing_id");
	}elseif($seq_type == "total_RNA" or $seq_type == "mRNA" or $seq_type == "scRNA" or $seq_type == "sequencing_only"){
		$log_name=$folder_name.$sequencing_id."_tmp.log";
		echo "the sequencing type is $seq_type, bcl2fastq pipeline would be started for fastq generation!<br/>";
		shell_exec("sudo bash /home/genomics/bin/bcl2fastq.sh $sequencing_id >> /var/www/html/log/$log_name &");	
	}
}
