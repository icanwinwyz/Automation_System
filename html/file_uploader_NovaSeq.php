...
if(preg_match("/\s+/", $fileName))
    {
	shell_exec("sudo rm /var/www/html/upload/tmp/*");
	echo '<span style="color:#F00;text-align:center;">ERROR: No space allowed in name of sample sheet. Please rename the sample sheet into \"SampleSheet.csv\" as example.</span>';
	exit();
}
...
$return_value=shell_exec("sudo perl /var/www/html/script/samplesheet_format_verify.pl $target_path /var/www/html/upload/tmp/$filename NovaSeq");

echo "the error code is $return_value <br><br>";

if($return_value == 1){
    echo '<span style="color:#F00;text-align:center;">ERROR: The number of projects input doesnt equal to the number of projects in Sample Sheet! Please double check or check whether the project ID you input is exactly same as in Sample Sheet!<br><br></span>';
    shell_exec("sudo rm /var/www/html/upload/tmp/*");
    exit();
}
else
if($return_value == 2){
    echo '<span style="color:#F00;text-align:center;">ERROR: Sample names should not include any sign like "+" or "\" or "/" or "." in sample sheet, please correct it.</span>';
    shell_exec("sudo rm /var/www/html/upload/tmp/*");
    exit(); 
}
else
if($return_value == 3){
    echo '<span style="color:#F00;text-align:center;">ERROR1: Sample names should not include any sign like "+" or "\" or "/" or "." in sample sheet, please correct it.<br></span>';
...
$cmd="sudo bash /var/www/html/script/mv_NovaSeq.sh $sample_sheet $sequencing_id NovaSeq";
...
if ($returnvalue != 0){
	...
	echo "<br/><br/>";
	 ...
	 $text="Fastq generation has been started for sequencing $sequencing_id!\r\n\n";
         $message=$text.$a.$b.$c.$h.$d.$e.$i.$f.$g;
	 $headers = "From: root";
...
if($fastq_ge == "Yes"){
      ...
      if($seq_type[0] == "scRNA_10X"){
             ...
             shell_exec("sudo bash /home/genomics/bin/cellranger_NovaSeq.sh NovaSeq $sequencing_id >> /var/www/html/log/$log_name &");
#       echo ("sudo bash /home/genomics/bin/cellranger.sh $folder_name $sequencing_id");
        }elseif($seq_type[0] == "Total_RNA" or $seq_type[0] == "ATAC" or $seq_type[0] == "mRNA" or $seq_type[0] == "scRNA" or $seq_type[0] == "miRNA"){
                $log_name=$sequencing_id."_tmp.log";
                echo "the sequencing type is ".implode(", ",$seq_type)." bcl2fastq pipeline starts for fastq generation!<br/>";
                shell_exec("sudo bash /home/genomics/bin/bcl2fastq_NovaSeq.sh NovaSeq $sequencing_id  >> /var/www/html/log/$log_name &");
        }else{
