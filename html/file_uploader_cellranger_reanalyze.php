...
#		$cmd="sudo bash /var/www/html/script/cellranger_reanalyze.sh ".$path_fastq." ".$sample[$x]." >> test.log.txt &";
#		echo "$cmd <br><br>";
	#	shell_exec($cmd);
#		shell_exec($cmd);
#		shell_exec("sudo bash /var/www/html/script/cellranger_reanalyze.sh $path_fastq $sample[$x] >> test.log.txt &");
#		echo "everything is good";
		passthru("sudo bash /var/www/html/script/cellranger_reanalyze.sh $path_fastq $sample[$x] > /var/www/html/log/$sample[$x].reanalyze_log.txt &",$returnvalue);
		if ($returnvalue != 0){
       		echo "<br/><br/>";
